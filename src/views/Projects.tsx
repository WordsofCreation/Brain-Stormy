...
function normalizeStoredProject(
  ...
  const category =
    project.category &&
    (projectCategories as readonly string[]).includes(project.category)
      ? project.category
      : "Other";
  const status =
    project.status &&
    (projectStatuses as readonly string[]).includes(project.status)
      ? project.status
      : "Planning";
  const priority =
    project.priority &&
    (projectPriorities as readonly string[]).includes(project.priority)
      ? project.priority
      : "Medium";
  const title =
    project.title?.trim() ||
  ...
  const tasks = Array.isArray(project.tasks)
    ? project.tasks.map(normalizeTask).filter((task) => task.title.trim())
    : [];
  const ideaIds = Array.isArray(project.ideaIds)
    ? [...new Set(project.ideaIds.filter(Boolean))]
    : [];
  ...
  return {
    ...
    description:
      ...
      "Turn this promising idea into a structured plan with action steps.",
...
function normalizeTask(task: Partial<ProjectTask>): ProjectTask {
  return {
    ...
    contactIds: Array.isArray(task.contactIds)
      ? [...new Set(task.contactIds.filter(Boolean))]
      : [],
  };
}

function projectsNeedMigration(
  storedProjects: StoredProject[],
  normalizedProjects: Project[],
) {
  return storedProjects.some(
    (project, index) =>
      JSON.stringify(project) !== JSON.stringify(normalizedProjects[index]),
  );
}
...
export function Projects() {
  const [storedProjects, setProjects] = useLocalStorage<Project[]>(
    projectStorageKey,
    sampleProjects,
  );
  const [ideas] = useLocalStorage<Idea[]>(ideaStorageKey, sampleIdeas);
  const [, setCalendarItems] = useLocalStorage<CalendarItem[]>(
    calendarStorageKey,
    sampleCalendarItems,
  );
  ...
  const projects = useMemo(
    () =>
      storedProjects.map((project, index) =>
        normalizeStoredProject(project ?? {}, index),
      ),
    [storedProjects],
  );
  ...
  const availableIdeas = useMemo(
    () => ideas.filter((idea) => !projectForm.ideaIds.includes(idea.id)),
    [ideas, projectForm.ideaIds],
  );
  ...
  const projectStats = useMemo(() => {
    ...
    const averageProgress =
      projects.length > 0
        ? Math.round(
            projects.reduce(
              (total, project) => total + calculateProgress(project.tasks),
              0,
            ) / projects.length,
          )
        : 0;
  ...
  const deleteProject = (project: Project) => {
    const confirmed = window.confirm(
      `Delete “${project.title}”? Attached ideas and tasks will remain in storage only as their original ideas.`,
    );
    ...
    if (activeProjectId === project.id) {
      setActiveProjectId(
        projects.find((currentProject) => currentProject.id !== project.id)
          ?.id ?? "",
      );
    }
  ...
  const deleteTask = (projectId: string, taskId: string) => {
    ...
    const confirmed = window.confirm(
      `Delete “${task?.title ?? "this task"}”? This action step cannot be recovered.`,
    );
  ...
  const updateTaskNotes = (
    ...
    updateTask(projectId, taskId, (task) => ({ ...task, stickyNotes }));
  };
  ...
  const detachIdea = (ideaId: string) => {
    setProjectForm((form) => ({
      ...form,
      ideaIds: form.ideaIds.filter((currentIdeaId) => currentIdeaId !== ideaId),
    }));
  };
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.5fr]">
        ...
        >
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.28em] text-violet">
                {editingProject ? "Edit project" : "Create project"}
              </p>
              <h2 className="mt-2 text-xl font-semibold tracking-tight sm:text-2xl">
                {editingProject ? "Refine the plan" : "Shape the next plan"}
              </h2>
            </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-silver/60">
                  Connect inbox ideas using project.ideaIds.
                </p>
              </div>
              <span className="rounded-full bg-white/10 px-3 py-1 text-xs text-silver">
                {projectForm.ideaIds.length}
              </span>
            </div>
              ...
              {projectForm.ideaIds.length > 0 ? (
                projectForm.ideaIds.map((ideaId) => {
                  const idea = ideas.find(
                    (currentIdea) => currentIdea.id === ideaId,
                  );
                  return (
                    <div
                      className="flex items-center justify-between gap-3 rounded-2xl bg-navy/55 px-3 py-2 text-sm"
                      key={ideaId}
                    >
                      <span className="line-clamp-1 text-silver">
                        {idea?.title ?? "Missing idea"}
                      </span>
                      <button
                        className="text-silver/60 transition hover:text-rose-100"
                        onClick={() => detachIdea(ideaId)}
                        type="button"
                        aria-label={`Detach ${idea?.title ?? "missing idea"}`}
                      >
                        <X size={16} />
                      </button>
                    </div>
                ...
                <p className="rounded-2xl border border-dashed border-white/10 px-3 py-3 text-sm text-silver/55">
                  No ideas attached yet.
                </p>
              )}
            </div>
        ...
        <div className="grid gap-5">
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <MetricCard
              icon={CheckCircle2}
              label="Tasks done"
              value={`${projectStats.completedTasks}/${projectStats.taskCount}`}
            />
            <MetricCard
              icon={Target}
              label="Avg progress"
            ...
            {projects.length > 0 ? (
              ...
              >
                ...
                <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-silver/70">
                  Create the first project to convert your best ideas into
                  tasks, dates, and progress you can see.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {activeProject ? (
          <ProjectDetailDrawer
            ideas={ideas.filter((idea) =>
              activeProject.ideaIds.includes(idea.id),
            )}
            onAddTask={addTask}
      ...
      <AnimatePresence>
        {activeProject && selectedTaskId
          ? (() => {
              const selectedTask = activeProject.tasks.find(
              ...
              return selectedTask ? (
                <TaskWorkspace
                  contacts={contacts.filter(
                    (contact) =>
                      !contact.projectId ||
                      contact.projectId === activeProject.id ||
                      contact.relatedTaskIds.includes(selectedTask.id) ||
                      selectedTask.contactIds.includes(contact.id),
                  )}
                  ...
                  onToggle={() => toggleTask(activeProject.id, selectedTask.id)}
                  project={activeProject}
                  task={selectedTask}
                />
              ) : null;
...
function ProjectCard({
  ...
  return (
    <motion.article
      layout
      initial={
        isMobile ? { opacity: 0, y: 10 } : { opacity: 0, y: 24, scale: 0.97 }
      }
      animate={isMobile ? { opacity: 1, y: 0 } : { opacity: 1, y: 0, scale: 1 }}
      exit={
        isMobile ? { opacity: 0, y: -8 } : { opacity: 0, y: -14, scale: 0.96 }
      }
      transition={{
    ...
    >
      ...
      <div className="relative flex flex-wrap items-center gap-2">
        ...
        <span
          className={`rounded-full border px-3 py-1 text-xs font-semibold ${priorityStyles[project.priority]}`}
        >
          {project.priority}
        </span>
      </div>
      ...
      <button
        className="relative mt-5 block text-left"
        onClick={onOpen}
        type="button"
      >
        ...
        <p className="mt-3 line-clamp-3 text-sm leading-6 text-silver/75">
          {project.description}
        </p>
      </button>
      ...
      <div className="relative mt-5">
        <div className="mb-2 flex items-center justify-between text-xs text-silver/65">
          <span>
            {completedTasks} of {project.tasks.length} steps complete
          </span>
          <span>{progress}%</span>
        </div>
      ...
      <div className="relative mt-5 grid grid-cols-[1fr_auto_auto] gap-2">
        ...
        <button
          className="min-h-11 rounded-full border border-white/10 bg-white/5 px-4 text-silver transition hover:text-white"
          onClick={onEdit}
          type="button"
...
function ProjectDetailDrawer({
  ...
  return (
    <motion.div
      className="fixed inset-0 z-50 flex items-end justify-end bg-navy/70 backdrop-blur-sm sm:items-stretch"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      ...
      >
        <div className="mb-6 flex items-start justify-between gap-4">
          ...
          <button
            className="rounded-2xl border border-white/10 bg-white/10 p-3 text-silver transition hover:text-white"
            onClick={onClose}
            type="button"
        ...
        <p className="mt-5 text-sm leading-7 text-silver/78">
          {project.description}
        </p>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          ...
          <InfoTile
            icon={Flag}
            label="Created"
            value={formatDate(project.createdAt.slice(0, 10))}
          />
        </div>

        {project.notes ? (
          <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.05] p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-silver/55">
              Notes
            </p>
            <p className="mt-2 text-sm leading-6 text-silver/78">
              {project.notes}
            </p>
          </div>
        ) : null}
        ...
        <div className="mt-7 rounded-3xl border border-white/10 bg-white/[0.055] p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-silver/60">
                Ideas connected through this project’s ideaIds.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-3">
            {ideas.length > 0 ? (
              ideas.map((idea) => (
                <div
                  className="rounded-2xl border border-white/10 bg-navy/45 p-3"
                  key={idea.id}
                >
                  ...
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-silver/65">
                    {idea.description}
                  </p>
                </div>
              ))
              ...
              <p className="rounded-2xl border border-dashed border-white/10 px-3 py-3 text-sm text-silver/55">
                No ideas attached to this project yet.
              </p>
            )}
          </div>
        </div>
        ...
        <form
          className="mt-7 rounded-3xl border border-white/10 bg-white/[0.055] p-4"
          onSubmit={onAddTask}
        >
          ...
          <div className="grid gap-3">
            <AnimatePresence mode="popLayout">
              {project.tasks.length > 0 ? (
                ...
                >
                  No action steps yet. Add the first one above to start tracking
                  progress.
                </motion.p>
              )}
            </AnimatePresence>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p
            className={`text-sm font-semibold ${task.completed ? "text-emerald-50 line-through decoration-emerald-200/70" : "text-white"}`}
          >
            {task.title}
          </p>
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-silver/60">
            {task.dueDate ? (
              <span className="rounded-full bg-navy/45 px-2.5 py-1">
        ...
        <button
          className="min-h-10 min-w-10 rounded-full text-silver/45 transition hover:bg-white/10 hover:text-violet"
          onClick={onOpen}
          type="button"
        ...
        <button
          className="min-h-10 min-w-10 rounded-full text-silver/45 transition hover:bg-white/10 hover:text-rose-100"
          onClick={onDelete}
          type="button"
