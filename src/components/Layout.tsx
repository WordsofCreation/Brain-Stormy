    ...
    <div className="min-h-screen overflow-x-hidden bg-navy bg-storm-radial text-white">
      ...
      <header className="fixed left-0 right-0 top-0 z-40 border-b border-white/10 bg-navy/70 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
          <button className="flex min-h-12 min-w-0 items-center gap-3" onClick={() => navigate('home')} type="button">
            ...
            <span className="min-w-0 text-left">
              <span className="block truncate text-base font-bold tracking-tight sm:text-lg">Brain Stormy</span>
              <span className="hidden text-xs uppercase tracking-[0.28em] text-silver/65 sm:block">Think · Shape · Schedule</span>
            </span>
          </button>
      ...
      <main className="relative z-10 mx-auto min-h-screen max-w-7xl px-4 pb-32 pt-24 sm:px-6 sm:pb-20 sm:pt-28 lg:px-8">{children}</main>

      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-navy/90 px-2 pb-[calc(env(safe-area-inset-bottom)+0.45rem)] pt-2 shadow-[0_-18px_60px_rgba(0,0,0,0.35)] backdrop-blur-2xl md:hidden" aria-label="Primary mobile navigation">
        <div className="mx-auto grid max-w-md grid-cols-6 gap-1 rounded-[1.35rem] border border-white/10 bg-white/[0.055] p-1">
          {navigationItems.map((item) => {
            const Icon = item.icon
            const active = item.id === activeView
            return (
              ...
              >
                {active ? <motion.span layoutId="bottom-nav-pill" className="absolute inset-0 rounded-2xl bg-white shadow-glow" transition={{ duration: 0.24, ease: 'easeOut' }} /> : null}
                <Icon className="relative" size={19} strokeWidth={active ? 2.4 : 2} />
